...
int main(int argc, char **argv)
{
  ...
  if (argc != 3) {
      RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "usage: Enter Your Graph as a sequence of edges in form «X-Y», where X and Y are nodes of the graph. Elements are separated with whitespace. ");
      return 1;
  }
 ...
 getline(cin, request->input);
  ...
  {
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), result.get()->output);
  } else {
    RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Failed to call mst_service");
  }
